<?php
	...
	for($i = 0; $i < $dataSize; $i++)
	{
	   ...
	   $result = $mysqli->query($sQuery);
    	...
    	{
			echo $insert_sp="INSERT INTO wallet_history (user_master_id,transaction_amt,status,notes,created_at,created_by) VALUES ('$user_id','$amount','Credited','Added money to wallet',NOW(),'$user_id')";
			...
			$result= mysqli_query($link, $insert_sph);
			 if (mysqli_num_rows($result) == 0) {
			   echo $wallet_query="INSERT INTO user_wallet (user_master_id,amt_in_wallet,total_amt_in_wallet,status,updated_at,updated_by) VALUES ('$user_id','$amount','$amount','Active',NOW(),'$user_id')";
			 }else{
				echo $wallet_query="UPDATE user_wallet SET amt_in_wallet=amt_in_wallet+'$amount',total_amt_in_wallet=total_amt_in_wallet+'$amount' WHERE user_master_id='$user_id'";
			 }
